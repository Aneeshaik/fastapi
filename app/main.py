...
from app.db import close_pool, get_db_conn, init_pool, queries
...
async def create_payment(amount: float, currency: str, db=Depends(get_db_conn)):
    payment_id = await queries.create_payment(db, amount=amount, currency=currency, status="pending")
    return {"id": payment_id}

@app.get("/list_payments")
async def list_payments(db=Depends(get_db_conn)):
    payments = await queries.list_payments(db)
